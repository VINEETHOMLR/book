...
class Btc extends Database {
    ...
    public static function attrs() {
        return ["id", "user_id", "transaction_type", "credit_type", "value", "before_bal", "after_bal", "created_at", "created_ip", "created_by", "coin_id","remarks"];
    }
    ...
    public function history($filter){
        ...
        $coinId = $this->callsql("SELECT id FROM coin WHERE coin_code='btc'",'value');
        ...
        if($filter['creditType']!=""){
          ...
          if($filter['creditType'] == 0){
            $search .= " AND credit_type = '0' ";
          }else{
            $search .="";
          }
        }
    ...
    public function historySum($filter){
      ...
      $search='';
        ...
        if(empty($filter['username']) && empty($filter['datefrom']) && empty($filter['dateto']) && empty($filter['txn_type']) && empty($filter['creditType']))
        {
           $search = "AND DATE(FROM_UNIXTIME(created_at)) = CURDATE()";
        }

        if(!empty($filter['creditType'])){
