...
class ExportWithdrawController extends Controller {
    ...
    public function actionExport(){
        ...
        $data  = $model->callsql("SELECT * FROM export_request_list WHERE id='$post_id' AND report = 3 AND status = 0","row");
        ...
        if(!empty($data)){
            ...
            $csv .= "Username, Receiver Address, Coin Type, Amount, Transaction Hash, Date, Status \n";//Column headers

            $totalPages = ceil($totalcount/$perPage);

            $filename_nw = 'withdraw_history_'.$post_id.'_'.$time_nw.'.csv';
    ...
    public function excelcreation($his){
        ...
        foreach ($his['data'] as $val) {
            ...
            $html.= $val['username'].','.$val['to_address'].','.$val['coin_name'].','.$val['amount'].','.$val['trans_hash'].','.$val['create_time'].','.$val['status']."\n"; //Append data to csv
