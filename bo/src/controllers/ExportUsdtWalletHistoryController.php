...
class ExportUsdtWalletHistoryController extends Controller {
    ...
    public function actionExport(){
        ...
        $data  = $model->callsql("SELECT * FROM export_request_list WHERE id='$post_id' AND report = 7 AND status = 0","row");
        ...
        if(!empty($data)){
            ...
            $csv .= "Username, Credit Type, Transaction Type, Amount, Before Balance, After Balance, Transaction Date, Remarks \n";//Column headers

            $totalPages = ceil($totalcount/$perPage);

            $filename_nw = 'usdt_wallet_history_'.$post_id.'_'.$time_nw.'.csv';
    ...
    public function getWhereQry($post_id,$data){
        ...
        $q['creditType'] = $data->creditType;
        $q['txn_type'] = $data->txn_type;
        $q['coin_code'] = $data->coin_code;
        $q['export'] = true;
    ...
    public function excelcreation($his){
        ...
        foreach ($his['data'] as $val) {
            ...
            $html.= $val['username'].','.$val['credit_type'].','.$val['txn_type'].','.$val['value'].','.$val['before_bal'].','.$val['after_bal'].','.$val['txn_date'].','.$val['remarks']."\n"; //Append data to csv
