...
class ExportDaywiseReportController extends Controller {
    ...
    public function actionExport(){
        ...
        $data  = $model->callsql("SELECT * FROM export_request_list WHERE id='$post_id' AND report = 4 AND status = 0","row");
        ...
        if(!empty($data)){
            ...
            $csv .= ", , Deposit, , ,Withdrawal, ,  \n";//Column headers
            $csv .= "Username, BTC,USDT,ETH,BTC,USDT,ETH \n";//Column headers

            $totalPages = ceil($totalcount/$perPage);

            $filename_nw = 'day_wise_report_'.$post_id.'_'.$time_nw.'.csv';
    ...
    public function excelcreation($his){
        ...
        foreach ($his['data'] as $val) {
            ...
            $html.= $val['username'].','.$val['depositBTC'].','.$val['depositUSDT'].','.$val['depositETH'].','.$val['withdrawlBTC'].','.$val['withdrawlUSDT'].','.$val['withdrawlETH']."\n"; //Append data to csv
