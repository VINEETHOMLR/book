...
class ExportTransferController extends Controller {
    ...
    public function actionExport(){
        ...
        $data  = $model->callsql("SELECT * FROM export_request_list WHERE id='$post_id' AND report = 9 AND status = 0","row");
        ...
        if(!empty($data)){
            ...
            $csv .= "Username, Coin Code,Amount,Credit Type, Transfer Date, Status \n";//Column headers

            $totalPages = ceil($totalcount/$perPage);

            $filename_nw = 'transfer_history_'.$post_id.'_'.$time_nw.'.csv';
    ...
    public function excelcreation($his){
        ...
        foreach ($his['data'] as $val) {
            ...
            $html.= $val['username'].','.$val['coin_code'].','.$val['amount'].','.$val['creditType'].','.$val['create_time'].','.$val['status']."\n"; //Append data to csv
